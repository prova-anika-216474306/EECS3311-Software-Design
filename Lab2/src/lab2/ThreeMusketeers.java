...
public class ThreeMusketeers {
	...
	public enum GameMode {
		Human("Human vs Human"), HumanRandom("Human vs Computer (Random)");
	...
	private void selectMode(GameMode mode) {
		...
		case HumanRandom -> {
			...
			// The following statement may look weird, but it's what is known as a ternary
			// statement.
			// Essentially, it sets musketeerAgent equal to a new HumanAgent if the value M
			// is entered,
	...
	/**
	 * Runs the game, handling human input for move actions Handles moves for
	 * different agents based on current turn
	 */
	...
	protected void move(final Agent agent) {
		...
		moves.add(new Move(new Cell(m.fromCell), new Cell(m.toCell)));
		this.board.move(m);
	...
	private void undoMove() {
		//M IS THE LAST ELEMENT OF STACK
	 ...
	 * 
	 * @return the selected Human side for Human vs Computer games, 'M': Musketeer,
	 *         G': Guard
	 */
